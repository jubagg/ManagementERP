    ...
    <div class="form-group">
      ...
      <div class="input-group">
        ...
        <select class="form-control  {{$errors->has('artunneg') ? 'is-invalid' : ''}}" name="artunneg" id="artunneg">
            ...
            <option value="{{$un->anid}}"{{ old('artunneg')  == $un->anid ? 'selected' : '' }} >{{$un->andesc}}</option>
            @endforeach
        </select>
      </div>
    ...
    <div class="form-group">
      ...
      <div class="input-group ">
        ...
        <select class="form-control {{$errors->has('artiva') ? 'is-invalid' : ''}}" name="artiva" id="artiva">
            <option value="">Seleccionar</option>
            @foreach($iva as $i)
            <option value="{{$i->icid}}" {{ old('artiva')  == $i->icid ? 'selected' : '' }}>{{$i->icdesc}}</option>
            @endforeach
        </select>
      </div>
...
<div class="col-3">
    <div class="form-group">
        ...
        <div class="input-group">
            ...
            <input type="text" name="artbonificacion" id="artbonificacion" value="{{$errors->has('artbonificacion') ? '' : old('artbonificacion')}}" class="form-control {{$errors->has('artbonificacion') ? 'is-invalid' : ''}}" placeholder="" aria-describedby="helpId">
        </div>
...
<div class="col-3">
    <div class="form-group">
        ...
        <div class="input-group">
            <input type="date" name="artfecbon" id="artfecbon" value="{{$errors->has('artfecbon') ? '' : old('artfecbon')}}" class="form-control {{$errors->has('artfecbon') ? 'is-invalid' : ''}}" placeholder="" aria-describedby="helpId">
        </div>
